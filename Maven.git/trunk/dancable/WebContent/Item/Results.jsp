<%-- 
    Document   : Results
    Created on : Oct 28, 2014, 2:32:40 PM
    Author     : kapil
--%>
...
session.setAttribute("loginid",loginid);
   ...
   String order_id=request.getParameter("oid");
    ...
    int chkOd=fn.checkQuestOrderIdInTemp("imd_cart_order",order_id);
 %>
 ...
 <%
   if(chkOd==0){
     //  out.println("txntype: "+request.getParameter("txn_type"));
   shp=Double.parseDouble(request.getParameter("shp"));
   ...
   itb.setBill_l_name(request.getParameter("billname"));
     ...
     itb.setShip_state(request.getParameter("sstate22"));
     ...
     itb.setType(request.getParameter("txn_type"));
     
int sz=Integer.parseInt(request.getParameter("sz"));
item_bean itbm;
for(int i=0;i<sz;i++){
    itbm=new item_bean();
    itbm.setUser_id(loginid);
    ...
    itbm.setCat_id(Integer.parseInt(request.getParameter("catid"+i)));
...
if(loginid.equals("guest"))
{
    ...
    fun.del_guest_ord_from_imd_cart(order_id);
...
<%                
  try{
       ...
       String user=mail;
       //String dt="";
       String od=comLogin.jumbleData(order_id);
       ...
       msg=msg.append("<a target=\"_blank\" href='http://"+request.getServerName()+":"+request.getServerPort()+""+request.getContextPath()+"/download.do?method=downloadfiles&key="+user+"&dt="+od+"'>");
       msg=msg.append("http://"+request.getServerName()+":"+request.getServerPort()+""+request.getContextPath()+"/download.do?method=downloadfiles&key="+user+"&dt="+od+"</a></td></tr></table>");
       }
       ...
       //out.println("mail: "+mail);
            ...
            // mail to merchant
    MimeMessage msg2=new MimeMessage(sess); 
    StringBuffer mermsg=new StringBuffer("<table width='100%'><tr><td colspan='2'>***** Please DO NOT REPLY to this automated message. *****<br><br></td></tr>");
       mermsg=mermsg.append(sbb.toString());
       msg2.setFrom(new InternetAddress("sales@danceables.com"));
       InternetAddress[] addre2 = {new InternetAddress("danceables@hotmail.com")};
       msg2.setRecipients(Message.RecipientType.TO, addre2);
       msg2.setSubject("Customer Order Details");
       msg2.setSentDate(new Date());
       msg2.setContent(mermsg.toString(), "text/html; charset=utf-8");
...
Thank you for visiting us. I hope to see you real soon and don’t forget to tell a friend.
 ...
 %>
            ...
            <tr><td>
      <table width="100%">
        <tr><td align="center"><font style="font-size: 20px" color="#E44129">Your Order Is Already Placed for order id <%=order_id%>.</font></td></tr>
        </table>
                </td></tr>      
            <%}%>
</table>
