 ...
 * @version
 */
public class Assign_Submit_fileup extends HttpServlet {
...
String fgname="";//f stands for fianl
String fassname="";
String fprofname="";
String fsender="";
String filename=""; 
...
//out.println(path1);
 ...
 //out.println(length);
   ...
   if(length > maxSize)
    ...
    RequestDispatcher rd=req.getRequestDispatcher("/College_Files/Assignment_Submit_fileup.jsp");
    rd.forward(req,res);
  ...
  try {
     ...
     // out.println("<PRE>");
       ...
       fgname=multi.getParameter("sg1");
      fassname=multi.getParameter("sn1");
      fprofname=multi.getParameter("sto");
  ...
  sub_final_con =(Connection)upload.Dataconnect();
     //uploadcon =(Connection)upload.getConnection(poolingEnabled);
        String qu1=" update ex_assignment_file set submit_file=?,submit_user=?,status=? where assign_group='"+fgname+"' and assign_name='"+fassname+"'";
     ...
     sub_final_prep.setString(1,filename);
      sub_final_prep.setString(2,fsender);
        sub_final_prep.setString(3,"1");
