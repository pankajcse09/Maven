...
public class StudInfoUpdate extends HttpServlet {
   ...
   { 
    ...
    con=(Connection)newsdc.Dataconnect();
       ...
       String cyr="";
       String ssn="";         
       String qr1="select studid,eyear,currentyr,sessions,max(To_NUMBER(sem)) as sem from studinfo where degree='"+dg+"' and TO_NUMBER(substr(sessions,1,4))=(select max(TO_NUMBER(substr(sessions,1,4))) from studinfo where degree='"+dg+"') group by studid,eyear,currentyr,sessions";
       ...
       hm=iu.update(dg,sid,sm,cyr,ssn); 
       qr3="update studinfo set status='"+hm.get("status").toString()+"' where studid='"+sid+"' and sem='"+sm+"' and sessions='"+ssn+"'";
       psmt3=con.prepareStatement(qr3);       
       psmt3.executeUpdate();  
       
       qr2="insert into studinfo(studid,eyear,sem,currentyr,sessions,degree,status,dated)values(?,?,?,?,?,?,?,?)";    
       psmt2=con.prepareStatement(qr2);
       psmt2.setString(1,sid);
       psmt2.setString(2,eyr);        
       psmt2.setString(3,hm.get("semes").toString());
       psmt2.setString(4,hm.get("crntyr").toString());
       psmt2.setString(5,hm.get("sesn").toString());
       psmt2.setString(6,dg);
       psmt2.setString(7,"N");
       psmt2.setString(8,dts);
       psmt2.executeUpdate();      
       ...
       res.sendRedirect(req.getContextPath()+"/UpdateStudInfo.jsp");       
       }
